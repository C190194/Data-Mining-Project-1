...
class Block:
    def __init__(self, data):
        ...
        self.number_of_classes = len(set(classes)) # get the number of classes
...
def calculate_entropy(data):
    ...
    class_count = dict([(label, 0) for label in classes]) # a dictionary wit class label and it's count
    for data_case in data:
        class_count[data_case[1]] += 1      # count the number of data case of each class
    entropy = 0
    for c in classes:
        p = class_count[c] / number_of_data
        entropy -= p * math.log2(p)         # calculate information entropy by its formula, where the base of log is 2
    return entropy
...
def split(block):
    ...
    candidates = list(set(candidates))          # get distinct values in the table
    candidates.sort()                           # sort the values in ascending order
    ...
    for value in candidates:      # test every cantidate if the entropy gain has met the minimum threshold
        ...
        # if entropy gain is greater than or equal to the minimum gain threshold, the value is an acceptable candidate for boundary
        if gain >= threshold:
            wall.append([value, gain, left_block, right_block])   # add the value to the wall

    if wall:    # has candidate
        wall.sort(key=lambda wall: wall[1], reverse=True)   # sort the value in descending order by "gain"
        return wall[0]      # return best candidate with maximum entropy gain
    else:
        return None         # there's no need to split the data
...
def partition(block):
    ...
    # sub_block: just a subset of the data block
    def recursive_split(sub_block):
        ...
        if wall_returned:                       # still can be spilt
            walls.append(wall_returned[0])      # record this partitioning value: the best candidate with maximum entropy gain
    ...
    recursive_split(block)      # call inner recursive function
    walls.sort()                # sort the boundaries in aescending order
    return walls
